...
const Property = ({property}) => {
    ...
    const {image, type, country, address, bedrooms, bathrooms, surface, price} = property
    ...
    return (
        <div className='bg-white shadow-lg p-5 rounded-lg rounded-tl-[90px] w-full max-w-[352px] mx-auto cursor-pointer hover:shadow-2xl transition'>
            ...
            <div className='mb-4 flex gap-x-2 text-sm'>
                <div className='bg-blue-400 text-white px-3 rounded-full'>{type}</div>
                <div className='bg-green-500 text-white px-3 rounded-full'>{country}</div>
            </div>
            ...
            <div className='text-lg font-semibold text-emerald-500'>{`$${price}`}</div>
        </div>
    )
}
